...
@ANTHROPIC_ROUTER.post("/models/available", response_model=List[str], responses={401: {"description": "Unauthorized"}, 403: {"description": "Forbidden"}})
async def get_available_models(
    api_key: Annotated[str, Body(embed=True)],
    current_user: str = Depends(get_current_user)
):
    """获取用户可用的 Anthropic 模型列表
    
    Args:
        api_key: Anthropic API 密钥，从请求体中获取
        current_user: 当前已认证的用户（通过依赖注入自动获取）
        
    Returns:
        List[str]: 用户可用的 Anthropic 模型 ID 列表
