...
async def generate_html_from_slide(
    ...
    """
    从幻灯片图像和XML内容生成HTML内容。

    参数:
        base64_image: 幻灯片图像的Base64编码字符串
        media_type: 图像的MIME类型（例如，'image/png'）
        xml_content: 幻灯片的XML内容（OXML格式）
        api_key: OpenAI API密钥
        fonts: 可选的字体列表，用于指定在输出中优先使用的字体家族（归一化后的根字体家族）
    ...
    try:
        ...
        if comparegpt_api_url:
            client = OpenAI(api_key=api_key, base_url=comparegpt_api_url)
        else:
            client = OpenAI(api_key=api_key, base_url="https://comparegpt.io/api")

        # 构建输入负载，包含系统提示、图像（单独）、OXML和可选字体文本
...
async def generate_react_component_from_html(
    ...
    """
    将HTML内容转换为TSX React组件。

    参数:
        html_content: 生成的HTML内容字符串
        api_key: CompareGPT API密钥
        image_base64: 可选的幻灯片图像Base64编码字符串（用于上下文）
        media_type: 可选的图像MIME类型（例如，'image/png'）（用于上下文）
...
async def edit_html_with_images(
    ...
    """
    根据一个或两个图像和文本提示编辑HTML内容。

    参数:
        current_ui_base64: 当前UI图像的Base64编码字符串
        sketch_base64: 可选的草图/指示图像的Base64编码字符串（用于上下文）
        media_type: 图像的MIME类型（例如，'image/png'）
        html_content: 要编辑的当前HTML内容
        prompt: 描述更改的文本提示
        api_key: CompareGPT API密钥
...
@SLIDE_TO_HTML_ROUTER.post("/", response_model=SlideToHtmlResponse, responses={401: {}, 403: {}})
async def convert_slide_to_html(
    ...
    """
    将幻灯片图像和OXML数据转换为HTML。
    
    参数:
        request: JSON请求包含图像路径和XML内容
        current_user: 当前登录用户ID（可选）
    
    返回:
        SlideToHtmlResponse包含生成的HTML
    """
    try:
        # 使用CompareGPT API key
        if not api_key:
            raise HTTPException(
                status_code=500, detail="Can't get CompareGPT API key"
            )

        # 图片路径为CompareGPT服务器上的路径
        image_path = request.image

        # 处理CompareGPT服务器上的图片路径
        if image_path.startswith("/app_data/images/"):
            # 删除CompareGPT服务器上的/app_data/images/前缀
...
@HTML_TO_REACT_ROUTER.post("/", response_model=HtmlToReactResponse, responses={401: {}, 403: {}})
async def convert_html_to_react(
    ...
    参数:
        request: JSON请求包含HTML内容
        current_user: 当前登录用户ID（可选）
    
    返回:
        HtmlToReactResponse包含生成的React组件
    """
    try:
        # 使用CompareGPT API key
        if not api_key:
...
# ENDPOINT 3: 编辑HTML内容（包含图片）
@HTML_EDIT_ROUTER.post("/", response_model=HtmlEditResponse, responses={401: {}, 403: {}})
async def edit_html_with_images_endpoint(
    ...
    """
    根据当前UI图像、草图/指示图像和编辑提示编辑HTML内容。
    
    参数:
        current_ui_image: 当前UI图像文件
        sketch_image: 草图/指示图像文件（可选）
        html: 要编辑的HTML内容（表单数据）
        prompt: 编辑提示（表单数据）
        current_user: 当前登录用户ID（可选）
    
    返回:
        HtmlEditResponse包含编辑后的HTML
    """
    try:
        # 使用CompareGPT API key
        if not api_key:
...
async def save_layouts(
    ...
    参数:
        request: JSON请求包含布局数据数组
        session: 异步数据库会话
        current_user: 当前登录用户ID（必选）
    
    返回:
        SaveLayoutsResponse包含成功状态和保存的布局数量
    """
    try:
        # 验证请求数据不能为空
        ...
        if len(request.layouts) > 50:  # 限制最多保存50个布局
            raise HTTPException(
...
async def get_layouts(
    ...
    参数:
        presentation: 演示文稿唯一标识符
        session: 异步数据库会话
        current_user: 当前登录用户ID
    
    返回:
        GetLayoutsResponse包含布局数据数组
    """
    try:
        # 验证演示文稿ID不能为空
...
async def create_template(
    ...
    参数:
        request: 包含模板ID、名称和描述的请求体
        session: 异步数据库会话
        current_user: 当前登录用户ID
    
    返回:
        TemplateCreateResponse包含创建或更新的模板信息
    """
    try:
        ...
        if existing:
            # 检查用户是否有权限更新此模板
            if existing.user_id and existing.user_id != current_user:
...
async def delete_template(
    ...
    try:
        # 检查模板是否存在
        existing = await session.get(TemplateModel, template_id)
        if not existing:
